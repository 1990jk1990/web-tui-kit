...
Tagged pre-1.0 releases are active. Security fixes are expected to target the latest published pre-1.0 release and the active development line. After `v0.7.0` is published, it becomes the maintained pre-1.0 release line; `v0.6.0` and older tags remain available for auditability but are not long-term support branches.
