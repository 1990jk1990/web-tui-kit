...
The project is pre-1.0. Version `0.7.0` is the current published prerelease and packages the established visual foundation, core dialog patterns, accessibility/keyboard refinement, canonical Chromium visual regression, deterministic release/distribution workflow, React/Vue/server-rendered consumption recipes, browser-driven Chromium/Firefox interaction verification with representative narrow touch behavior, and browser-driven accessibility-semantic verification of canonical roles, names, labels, native states, and progress semantics. The active v0.8 milestone adds executable compile/runtime evidence for the canonical framework/template recipes with pinned representative tool versions while preserving the framework-independent runtime and tagged-vendoring model. Linux Chromium remains the sole screenshot-baseline authority; physical Android and real assistive-technology/WCAG verification remain separate evidence domains. Public component contracts may still evolve through minor releases before 1.0.
