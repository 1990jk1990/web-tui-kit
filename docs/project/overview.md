...
The project is pre-1.0. Version `0.8.0` packages the established visual foundation, core dialog patterns, accessibility/keyboard refinement, canonical Chromium visual regression, deterministic release/distribution workflow, React/Vue/server-rendered consumption recipes, browser-driven Chromium/Firefox interaction verification with representative narrow touch behavior, browser-driven accessibility-semantic verification of canonical roles, names, labels, native states, and progress semantics, and executable compile/runtime verification for the canonical React/Vue/server-rendered recipes with pinned representative framework/tool versions. The framework-independent `src/` runtime and immutable tagged-vendoring model remain unchanged. Linux Chromium remains the sole screenshot-baseline authority; pinned framework versions are representative evidence rather than an exhaustive support matrix, and physical Android plus real assistive-technology/WCAG verification remain separate evidence domains.

The active v0.9 readiness work formalizes which parts of that established runtime are intentionally public and how compatibility will be handled. Before 1.0, public component contracts may still change in a MINOR release when the migration is explicit. Once 1.0 is deliberately published, normal Semantic Versioning applies to the declared public contract.
