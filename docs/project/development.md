...
demo/                executable visual and semantic references
...
tests/               structural/release/visual/browser verification evidence
scripts/             validation, visual/interaction/accessibility regression, and release tooling
...
4. Update the demo for new or changed reusable visual/semantic patterns; update `examples/` when consumption guidance changes.
...
Do not introduce one-off styling when an existing token or pattern serves the same purpose. If a genuinely reusable token or component is added, keep its implementation, specification, demo coverage, practical design-system guidance, visual baselines, applicable interaction evidence, and applicable accessibility-semantic evidence aligned.
...
Canonical roles, names, label associations, native state, and progress semantics are exercised separately through `scripts/accessibility_regression.py`, which runs against `demo/index.html` and `demo/dialogs.html`. Prefer native HTML semantics and keep presentation-only `.tui-dialog` styling distinct from actual modal semantics.

Chromium remains the canonical screenshot baseline environment. Interaction and accessibility-semantic compatibility evidence additionally exercises Firefox desktop and a narrow touch-capable Chromium context. Keep Playwright/browser dependencies out of `src/`; they are development and release-gate tooling only.

Compatibility claims belong in `docs/project/compatibility.md`. Physical Android results must come from an actual run recorded through `docs/project/android-device-check.md`, not from browser emulation. Browser role/name assertions likewise do not establish screen-reader behavior or WCAG conformance.

## Release preparation

Release/version changes are not routine development metadata. Follow `RELEASING.md`; update `VERSION` and `CHANGELOG.md` together, run structural/docs/visual/interaction/accessibility-semantic release gates, validate the deterministic distribution archive, and keep published tags immutable. Direct tagged vendoring is the current distribution baseline.
